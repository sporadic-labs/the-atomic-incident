...
class PostProcessor extends Phaser.Sprite {
    ...
    onHealthUpdate(health = 0) {
        // RGB split
        if (health <= 0) {
            this.rgbSplitFilter.setDisplacement(15).setLoopInterval(500).loopSplit();
        } else if (health <= 0.4) {
            const displacement = Phaser.Math.mapLinear(health, 0, 0.4, 15, 5);
            const interval = Phaser.Math.mapLinear(health, 0, 0.4, 800, 1500);
            this.rgbSplitFilter.setDisplacement(displacement).setLoopInterval(interval).loopSplit();
        } else {
