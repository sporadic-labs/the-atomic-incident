...
export class AmmoManager extends Phaser.Group {
  ...
  constructor(game, parentGroup) {
    ...
    this._text = {
      r: game.make.text(x, y, 0, { font, fill: Color.red().getWebColor() }),
      g: game.make.text(x + 1 * spacing, y, 0, { font, fill: Color.green().getWebColor() }),
      b: game.make.text(x + 2 * spacing, y, 0, { font, fill: Color.blue().getWebColor() })
    };
  ...
  getAmmoByColor(color) {
    if (color === Color.red()) return this._ammo.r;
    else if (color === Color.green()) return this._ammo.g;
    else if (color === Color.blue()) return this._ammo.b;
    else return null;
  }
  ...
  incrementAmmoByColor(color, delta) {
    if (color === Color.red()) this._ammo.r += delta;
    else if (color === Color.green()) this._ammo.g += delta;
    else if (color === Color.blue()) this._ammo.b += delta;
    this._updateText();
    return this;
  }
