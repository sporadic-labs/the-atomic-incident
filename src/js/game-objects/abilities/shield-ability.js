...
class ShieldAbility extends Ability {
  ...
  update() {
    ...
    if (this._shieldCooldown.getCooldownProgress() > 0.75 && !this._fading) {
      this._fading = true;
      this._shieldTween = this.game.make
        .tween(this._shield.baseColor)
  ...
  activate() {
    this._redKey.onDown.add(() => this._trigger(Color.red()), this);
    this._greenKey.onDown.add(() => this._trigger(Color.green()), this);
    this._blueKey.onDown.add(() => this._trigger(Color.blue()), this);
    super.activate();
  }
